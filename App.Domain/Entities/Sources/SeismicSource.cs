...
namespace App.Domain.Entities.Sources
{
    public class SeismicSource
    {
        ...
        private readonly List<string> _storyNames = new List<string>();
        public IReadOnlyList<string> StoryNames => _storyNames.AsReadOnly();

        private SeismicSource() { }

        public SeismicSource(Guid projectId, string label, string sapModelPath, HydrationPurpose purpose)
        {
            Id = Guid.NewGuid();
            ProjectId = projectId;
        ...
        public void AddStoryName(string story)
        {
            if (!string.IsNullOrWhiteSpace(story))
                _storyNames.Add(story);
        }
    }
}
